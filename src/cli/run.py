    ...
    try:
        ...
        try:
            ...
            discovery_coordinator = DiscoveryCoordinator(
                smarttub_client=smarttub_client, config=config
            )
            ...
            if discovery_mode == "startup_quick":
                logger.info(
                    "Starting background discovery (quick mode) via DISCOVERY_MODE env var..."
                )
                await discovery_coordinator.start_discovery(mode="quick")
            elif discovery_mode == "startup_full":
                logger.info(
                    "Starting background discovery (full mode) via DISCOVERY_MODE env var..."
                )
                await discovery_coordinator.start_discovery(mode="full")
            elif discovery_mode == "startup_yaml":
                logger.info(
                    "Starting background discovery (yaml_only mode) via DISCOVERY_MODE env var..."
                )
                await discovery_coordinator.start_discovery(mode="yaml_only")
            else:
                logger.info(
                    f"Discovery mode: {discovery_mode} (manual control via WebUI or MQTT)"
                )

        except Exception as e:
            logger.error(
                f"Failed to initialize background discovery: {e}", exc_info=True
            )
            error_tracker.track_error(
        ...
        if (
            not discover
            and not show_discovery
            and not getattr(config, "check_smarttub", True)
        ):
            broker.publish(
                f"{config.mqtt.base_topic}/status",
            ...
            if (
                getattr(config, "check_smarttub", True)
                and not discover
                and not show_discovery
            ):
                logger.info(
                    "Starting initial discovery in background (CHECK_SMARTTUB=true)"
                )
        ...
        if discover:
            ...
            try:
                ...
                item_prober = ItemProber(
                    config, smarttub_client, topic_mapper, error_tracker=error_tracker
                )
                await item_prober.probe_all()
        ...
        if config.web.enabled and HAS_WEB and HAS_UVICORN:
            ...
            logger.warning(
                "Web UI dependencies not available - install FastAPI and Jinja2"
            )
        ...
        capability_refresh_task = loop.create_task(
            _capability_refresh_loop(
                capability_detector, config.smarttub.polling_interval_seconds * 5, event
            )
        )
