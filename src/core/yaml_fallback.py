    ...
    async def publish_from_yaml(
        ...
        try:
            ...
            if not yaml_path.exists():
                logger.warning(f"YAML file not found: {yaml_path} - skipping fallback publishing")
                return False
            ...
            if not data or 'discovered_items' not in data:
                logger.warning(f"Invalid YAML structure in {yaml_path} - missing 'discovered_items' key")
                return False
