    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery": {
                ...
                "timestamp": datetime.fromtimestamp(self.recovery_timestamp, tz=timezone.utc).isoformat() 
                            if self.recovery_timestamp else None
            }
        }
...
class ErrorTracker:
    ...
    def attempt_recovery(
        ...
        with self._lock:
            recent_errors = [e for e in reversed(self._errors) if e.category == category and not e.recovery_attempted]
            ...
            if recovery_action:
                ...
                for callback in callbacks:
                    try:
                        ...
                        logger.warning(f"Recovery callback failed for {category.value}: {e}")
        ...
        logger.info(f"Recovery {'succeeded' if success else 'failed'} for {category.value}")
        return success
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            error_count = sum(1 for e in self._errors if e.severity == ErrorSeverity.ERROR)
        
        return {
            "total_errors": total_errors,
            "critical_count": critical_count,
            "error_count": error_count,
            "by_category": {cat.value: count for cat, count in category_counts.items() if count > 0},
    ...
    def clear_errors(self, category: Optional[ErrorCategory] = None) -> int:
        ...
        logger.info(f"Cleared {cleared} errors" + (f" for category {category.value}" if category else ""))
        return cleared
        ...
        with self._lock:
            for category in ErrorCategory:
                ...
                if not recent_errors:
                    ...
                    # Check severity
                    has_critical = any(e.severity == ErrorSeverity.CRITICAL for e in recent_errors)
                    has_error = any(e.severity == ErrorSeverity.ERROR for e in recent_errors)
