...
class SmartTubClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not self.config.smarttub.device_id and self._spas:
                ...
                logger.info(
                    f"Auto-detected SmartTub device ID: {self.config.smarttub.device_id}"
                )

            logger.info(
                f"Connected to SmartTub account with {len(self._spas)} spa(s), _spas: {self._spas}"
            )
    ...
    async def get_state_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if pumps_data and isinstance(pumps_data, list):
                    ...
                    for pump in pumps_data:
                        ...
                        pump_state = (
                            pump.state.name
                            if hasattr(pump.state, "name")
                            else str(pump.state)
                        )
                        pump_entry = {
                            "id": pump.id,
                            "type": pump.type.name
                            if hasattr(pump.type, "name")
                            else str(pump.type),
                            "state": "running"
                            if pump_state in ("HIGH", "ON")
                            else "off",
                            "speed": "ONE_SPEED",  # Default for most pumps
                        }
                        snapshot["components"]["pumps"].append(pump_entry)
                        logger.debug(
                            f"Added pump {pump.id} (state={pump_state}) to snapshot"
                        )
                else:
                    logger.warning(
            ...
            try:
                ...
                lights_data = (
                    lights_response.get("lights", [])
                    if isinstance(lights_response, dict)
                    else []
                )
                ...
                if lights_data and isinstance(lights_data, list):
                    ...
                    for light_dict in lights_data:
                        ...
                        logger.debug(
                            f"Light color_obj: type={type(color_obj)}, value={color_obj}, isinstance_dict={isinstance(color_obj, dict)}"
                        )
                        if isinstance(color_obj, dict):
                            ...
                            logger.debug(
                                f"Color from dict: R={red}, G={green}, B={blue}, raw={color_obj}"
                            )
                        else:
    ...
    async def set_pump_state(
        self, enabled: bool, pump_id: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            if not target_pump:
                logger.warning(
                    f"Pump '{pump_id}' not found",
                    extra={"available_pumps": [p.id for p in pumps]},
                )
                return
            ...
            logger.debug(
                f"Pump {target_pump.id} current state: {target_pump.state.name} (is_on={current_state_is_on})"
            )

            # Only toggle if state needs to change
            if current_state_is_on == enabled:
                logger.info(
                    f"Pump {target_pump.id} already in desired state ({target_pump.state.name})"
                )
                return

            # Toggle the pump
            logger.info(
                f"Toggling pump {target_pump.id} from {target_pump.state.name} to {'ON' if enabled else 'OFF'}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Successfully sent toggle request for pump {target_pump.id}"
                )
    ...
    async def set_light_mode(self, mode: str, light_id: str | None = None) -> None:
        ...
        try:
            ...
            await spa.request(
                "PATCH",
                f"lights/{target_light.zone}",
                {"mode": mode_upper, "intensity": intensity},
            )
            logger.info(
                f"Set light zone {target_light.zone} mode to {mode_upper} (intensity={intensity})"
            )
        except Exception as e:
            logger.error(
                f"Failed to set light mode for zone {target_light.zone}: {e}",
                exc_info=True,
            )
