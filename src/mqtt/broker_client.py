...
class MQTTBrokerClient:
    ...
    def connect(self) -> None:
        ...
        if hasattr(client, "reconnect_delay_set"):
            client.reconnect_delay_set(min_delay=self._reconnect_min, max_delay=self._reconnect_max)

        host, port = self._resolve_endpoint(self._mqtt_config.broker_url)
        keepalive = getattr(self._mqtt_config, "keepalive_seconds", self.DEFAULT_KEEPALIVE_SECONDS)

        self._current_backoff = self._reconnect_min

        # Try to connect and log helpful diagnostics on failure.
        try:
            self._logger.info("Attempting MQTT connect", extra={"host": host, "port": port, "keepalive": keepalive})
            ...
            self._logger.error("Failed to connect to MQTT broker", exc_info=exc, extra={"host": host, "port": port})
            raise
    ...
    def publish(self, topic: str, payload: Any = None, *, qos: int | None = None, retain: bool | None = None) -> Any:
        ...
        try:
            ...
            self._logger.error("mqtt-publish-failed", exc_info=exc, extra={"topic": topic})
            raise
                ...
                try:
                    ...
                    if best_callback:
                        try:
                            ...
                            self._logger.error(f"Error in MQTT callback for topic {received_topic}: {e}", exc_info=True)
                    else:
                        self._logger.debug(f"No callback matched for topic: {received_topic}")
                except Exception as e:
                    self._logger.error(f"Error in MQTT message handler: {e}", exc_info=True)
    ...
    def publish_meta_mqtt(self) -> None:
        ...
        meta_data = {
            ...
            "interface": {
                ...
                "tls_enabled": getattr(getattr(self._mqtt_config, "tls", None), "enabled", False),
                "keepalive": getattr(self._mqtt_config, "keepalive", self.DEFAULT_KEEPALIVE_SECONDS),
    ...
    def _reconnect_min(self) -> int:
        return max(
            1,
            int(getattr(self._mqtt_config, "reconnect_min_seconds", self.DEFAULT_RECONNECT_MIN_SECONDS)),
        )
    ...
    def _reconnect_max(self) -> int:
        return max(
            self._reconnect_min,
            int(getattr(self._mqtt_config, "reconnect_max_seconds", self.DEFAULT_RECONNECT_MAX_SECONDS)),
        )
    ...
    def _create_client(self) -> Any:
        ...
        if self._mqtt_config.username:
            client.username_pw_set(self._mqtt_config.username, self._mqtt_config.password)
    ...
    # Callbacks
    # ------------------------------------------------------------------
    def _handle_connect(self, client: Any, userdata: Any, flags: Any, reason_code: Any, properties: Any) -> None:
        self._current_backoff = self._reconnect_min
        ...
        client_id = None
        try:
            raw_id = getattr(client, "client_id", None) or getattr(client, "_client_id", None)
    ...
    def _handle_disconnect(
        ...
        if not failure:
            ...
            self._logger.debug(
                ...
                extra={"client_id": client_id, "disconnect_flags": disconnect_flags, "properties": properties},
            )
            ...
            try:
                ...
                self._logger.debug(f"Failed to publish MQTT meta on clean disconnect: {e}")
