...
async def test_topic_mapper_status():
    ...
    assert status_msg.topic == "smarttub-mqtt/discovery/status", f"Wrong topic: {status_msg.topic}"
    ...
    assert result_msg.topic == "smarttub-mqtt/discovery/result", f"Wrong topic: {result_msg.topic}"
...
async def test_control_topic():
    ...
    assert control_topic == "smarttub-mqtt/discovery/control", f"Wrong topic: {control_topic}"
...
async def test_mqtt_handler_lifecycle():
    ...
    assert call_args[1]["topic"] == "smarttub-mqtt/discovery/control", "Should subscribe to control topic"
...
async def test_control_commands():
    ...
    status_messages = [m for m in mqtt_client.published_messages if "status" in m["topic"]]
...
async def test_auto_publishing():
    ...
    status_messages = [m for m in mqtt_client.published_messages if "status" in m["topic"]]
    assert len(status_messages) >= 2, f"Should auto-publish status updates, got {len(status_messages)}"
