        ...
        "discovered_items": {
            "test-spa-1": {
                "lights": [
                    ...
                    {"id": "zone_2", "detected_modes": ["OFF", "ON"]},
                ]
            },
            "test-spa-2": {
                "lights": [
...
async def test_yaml_fallback_publishing():
    ...
    try:
        ...
        # Check payloads
        zone_1_msg = next(
            m for m in messages if "zone_1" in m["topic"] and "spa-1" in m["topic"]
        )
        assert zone_1_msg["payload"] == "OFF,ON,PURPLE,WHITE", (
            f"Wrong payload: {zone_1_msg['payload']}"
        )

        zone_2_msg = next(m for m in messages if "zone_2" in m["topic"])
        assert zone_2_msg["payload"] == "OFF,ON", (
            f"Wrong payload: {zone_2_msg['payload']}"
        )
...
async def test_empty_detected_modes():
    ...
    try:
        data = {
            "discovered_items": {
                "test-spa": {"lights": [{"id": "zone_1", "detected_modes": []}]}
            }
        }
