#!/usr/bin/env python3
"""
Validation script for Discovery Coordinator.

Tests the high-level API and integration between components.
"""
...
# Import via importlib to avoid dependency issues
...
discovery_state = load_module(
    "discovery_state",
    src_dir / "core" / "discovery_state.py"
)
background_discovery = load_module(
    "background_discovery", 
    src_dir / "core" / "background_discovery.py"
)
discovery_coordinator = load_module(
    "discovery_coordinator",
    src_dir / "core" / "discovery_coordinator.py"
)

# Get classes
DiscoveryCoordinator = discovery_coordinator.DiscoveryCoordinator
...
async def test_concurrent_start():
    ...
    assert "already running" in result2["error"].lower(), "Should indicate already running"
...
async def test_mqtt_publisher():
    ...
    assert len(published_states) >= 2, f"Should publish states, got {len(published_states)}"
